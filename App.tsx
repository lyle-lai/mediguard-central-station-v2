...
const App: React.FC = () => {
  ...
  useEffect(() => {
      if (!settings.isDemoMode && currentUser) {
          fetchGlobalConfig(selectedDepartment).then(config => {
              setSettings(prev => ({
                  ...prev,
                  // Merge partial updates
                  deviceConfigs: config.deviceConfigs ? { ...prev.deviceConfigs, [selectedDepartment]: config.deviceConfigs[selectedDepartment] } : prev.deviceConfigs,
                  deptCapacity: config.deptCapacity ? { ...prev.deptCapacity, [selectedDepartment]: config.deptCapacity[selectedDepartment] } : prev.deptCapacity,
                  alarmThresholds: config.alarmThresholds ? { ...prev.alarmThresholds, [selectedDepartment]: config.alarmThresholds[selectedDepartment] } : prev.alarmThresholds,
                  bedLabels: config.bedLabels ? { ...prev.bedLabels, [selectedDepartment]: config.bedLabels[selectedDepartment] } : prev.bedLabels
              }));
  ...
  const handleLogin = (user: User) => {
    setCurrentUser(user);
    const defaultDept = (user.departments && user.departments.length > 0) ? user.departments[0].code : Department.ICU;
  ...
  const handleSaveNewPatient = async (data: Partial<PatientData>) => {
      if (settings.isDemoMode) {
           ...
           try {
               ...
               // We rely on polling or websocket to get the new patient list update in real mode
           } catch (e) {
               addToast('入科失败', 'error');
           }
      }
