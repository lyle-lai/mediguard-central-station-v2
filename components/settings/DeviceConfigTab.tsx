...
const DeviceConfigTab: React.FC<DeviceConfigTabProps> = ({ config, onSave, activeTab, editingDevice, setEditingDevice }) => {
  ...
  const [localDeviceConfigs, setLocalDeviceConfigs] = useState<DeviceDisplayConfig>(() => JSON.parse(JSON.stringify(config)));
            ...
            <div className="flex-1 mb-16"> 
                ...
                <div className="bg-gray-900/30 rounded-lg p-1 space-y-1 border border-gray-800/50 overflow-y-auto max-h-[500px]">
                    {sortedDisplayList.map((c, idx) => (
                        <div key={c.id} className={`flex items-center justify-between p-2 rounded border transition group ${c.visible ? 'bg-gray-800 border-gray-600' : 'bg-gray-900/50 border-gray-800 opacity-60'}`}>
