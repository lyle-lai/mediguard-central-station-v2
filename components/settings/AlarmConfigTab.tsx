...
const AlarmConfigTab: React.FC<AlarmConfigTabProps> = ({ thresholds, config, onSave, nightMode, onToggleNightMode }) => {
  ...
  useEffect(() => {
      if (!isDirty) {
          ...
          if (config) {
              Object.values(config).forEach(deviceConfig => {
                  if (deviceConfig && deviceConfig.parameters) {
                      deviceConfig.parameters.forEach(p => {
                    ...
                    {localThresholds.map((rule, idx) => (
                        <tr key={rule.paramId} className="hover:bg-gray-800/50 transition group">
                            ...
                            <td className="p-3">
                                ...
                                className={`bg-gray-900 border border-gray-700 rounded px-2 py-1 text-xs font-bold outline-none cursor-pointer ${rule.priority === AlarmPriority.HIGH ? 'text-red-400' : rule.priority === AlarmPriority.MED ? 'text-yellow-400' : 'text-blue-400'}`}
                                >
                                    <option value={AlarmPriority.HIGH}>High (危急)</option>
                                    <option value={AlarmPriority.MED}>Med (警告)</option>
                                    <option value={AlarmPriority.LOW}>Low (提示)</option>
                                </select>
                            </td>
                            <td className="p-3">
