...
import { DepartmentPreferences, SystemSettings } from '../../types';
import { Database, Gauge, PlayCircle, AlertTriangle } from 'lucide-react';

interface SimulationControlProps {
  settings: SystemSettings;
  onUpdate: (newSettings: SystemSettings) => void; // Keeps updating local state visually
  onSavePreferences: (prefs: Partial<DepartmentPreferences>) => void; // Trigger save
  onTriggerAlarm?: () => void;
}

const SimulationControl: React.FC<SimulationControlProps> = ({ settings, onUpdate, onSavePreferences, onTriggerAlarm }) => {

  const handleToggleDemo = () => {
    const newVal = !settings.isDemoMode;
    // Optimistic update
    onUpdate({ ...settings, isDemoMode: newVal });
    // Save
    onSavePreferences({ isDemoMode: newVal });
  };

  const handleSpeedChange = (val: number) => {
    onUpdate({ ...settings, simulationSpeed: val });
  };

  const handleSpeedCommit = () => {
    onSavePreferences({ simulationSpeed: settings.simulationSpeed });
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 border-t border-gray-800/50">
          <div className="p-4 bg-gray-900/30 rounded-lg border border-gray-800">
            ...
            <input
              ...
              onMouseUp={handleSpeedCommit} // Save on release
