...
import { AppView, Department, SystemSettings, User, DepartmentDesc } from '../../types';
import { Building2, ChevronDown, LayoutGrid, Users, Bell, Settings, LogOut, Activity } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // CHANGED: Logic to check if user can switch department
  ...
  return (
    <aside className="w-16 flex flex-col items-center bg-med-card border-r border-gray-800 py-4 z-40 shrink-0 relative transition-all duration-300">
        ...
        <button 
            ...
            title={`当前科室: ${DepartmentDesc[selectedDepartment]}`}
        >
        ...
        {canSwitchDepartment && isDeptMenuOpen && (
            ...
            {(currentUser.departments || []).map(dept => (
                ...
                <div className="flex justify-between">
                    <span>{dept.name}</span>
                    <span className="text-xs bg-gray-800 px-1.5 py-0.5 rounded text-gray-500">{settings.deptCapacity[dept.code]}床</span>
                </div>
                </button>
            ))}
            </div>
        ...
        <nav className="flex flex-col gap-6 w-full">
            <button onClick={() => setCurrentView(AppView.DASHBOARD)} className={`h-12 w-full flex justify-center items-center transition relative ${currentView === AppView.DASHBOARD ? 'text-blue-400 bg-blue-400/10' : 'text-gray-500 hover:text-gray-300'}`}>
                {currentView === AppView.DASHBOARD && <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-400"></div>}
                <LayoutGrid size={24} />
            </button>
            
            <button onClick={() => setCurrentView(AppView.PATIENTS)} className={`h-12 w-full flex justify-center items-center transition relative ${currentView === AppView.PATIENTS ? 'text-blue-400 bg-blue-400/10' : 'text-gray-500 hover:text-gray-300'}`}>
                {currentView === AppView.PATIENTS && <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-400"></div>}
                <Users size={24} />
            </button>

            <button onClick={() => setCurrentView(AppView.ALARMS)} className={`h-12 w-full flex justify-center items-center transition relative ${currentView === AppView.ALARMS ? 'text-blue-400 bg-blue-400/10' : 'text-gray-500 hover:text-gray-300'}`}>
            ...
            <button onClick={() => setCurrentView(AppView.SETTINGS)} className={`h-12 w-full flex justify-center items-center transition relative ${currentView === AppView.SETTINGS ? 'text-blue-400 bg-blue-400/10' : 'text-gray-500 hover:text-gray-300'}`}>
            ...
            <div className="mt-auto mb-4 w-full flex justify-center">
                <button onClick={handleLogout} className="h-12 w-full flex justify-center items-center text-gray-500 hover:text-red-400 hover:bg-red-900/10 transition">
                    <LogOut size={20} />
                </button>
            </div>
    </aside>
