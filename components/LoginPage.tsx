  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isDemoMode) {
      ...
      try {
        ...
        // Validate user has departments
        if (!response.user.departments || response.user.departments.length === 0) {
          setError('登录失败：该账号未分配科室，请联系管理员');
          setIsLoading(false);
          return;
        }

        // Store token if needed, usually handled by App or Context
        if (response.token) {
        ...
        <div className="p-8 flex-1">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="demoMode" className="text-sm text-gray-400 cursor-pointer select-none">
                演示模式 (使用本地模拟数据)
              </label>
            </div>
